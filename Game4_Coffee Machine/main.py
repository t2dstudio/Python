...
def is_sufficient(customer_preference):
    for item in customer_preference:
        if customer_preference[item] > resources[item]:
...
while machine_is_running:
    ...
    if user_choice == 'off':
        ...
        if is_sufficient(selected_drink["ingredients"]):
            payment = process_coins()
            if is_transaction_successful(payment,selected_drink["cost"]):
                make_coffee(user_choice, selected_drink['ingredients'])
